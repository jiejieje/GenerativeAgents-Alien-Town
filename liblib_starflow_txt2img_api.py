...
# --- 冻结环境资源定位（与 start.py/replay.py 保持一致） ---
def _get_base_dir():
...
def _resolve_records_dir(subdir: str) -> str:
    ...
    1) BASE_DIR/results/<subdir>  （start.py 写入位置：与进程工作目录一致）
    2) RESOURCE_ROOT/results/<subdir>  （某些打包布局会放在 _internal 下）
    3) dirname(RESOURCE_ROOT)/results/<subdir>  （AI-Town/_internal 的上级）
    """
    # 优先使用外部注入的结果根目录（由启动器提供）
    try:
...
MODEL_CHECKPOINT_ID = "0ea388c7eb854be3ba3c6f65aac6bfd3" # 星流模型 Checkpoint ID (来自 参考.py)
MODEL_TEMPLATE_UUID = "e10adc3949ba59abbe56e057f20f883e" # 星流模型 Template UUID (来自 参考.py)
...
DEFAULT_TXT2IMG_WIDTH = 768 # 星流模型推荐尺寸 (来自 参考.py) - 改为768以匹配1:1
DEFAULT_TXT2IMG_HEIGHT = 768 # 星流模型推荐尺寸 (来自 参考.py) - 改为768以匹配1:1

DEFAULT_TXT2IMG_STEPS = 20

DEFAULT_TXT2IMG_CFG_SCALE = 7.0 # 星流模型推荐 (来自 参考.py)
DEFAULT_TXT2IMG_SAMPLER_NAME = "Euler a"
DEFAULT_TXT2IMG_IMG_COUNT = 1
DEFAULT_TXT2IMG_CLIP_SKIP = 2 # 星流模型推荐 (来自 参考.py)
...
DEFAULT_JSON_RECORDS_DIR = _resolve_records_dir("paint-records")
DEFAULT_JSON_FILENAME_STEM = "sim_20250520_165956" # 默认JSON文件名 (不含.json后缀)

# --- 配置覆盖：从 data/config.json 与环境变量读取 ---
try:
    with open(os.path.join(RESOURCE_ROOT, "data", "config.json"), "r", encoding="utf-8") as _cfg_f:
        _cfg = json.load(_cfg_f)
except Exception:
    _cfg = {}

_liblib_cfg = (_cfg.get("services", {}).get("liblibai", {}) if isinstance(_cfg, dict) else {})
if isinstance(_liblib_cfg, dict):
    LIBLIBAI_BASE_URL = os.getenv("LIBLIBAI_BASE_URL") or _liblib_cfg.get("base_url", LIBLIBAI_BASE_URL)
    _starflow_cfg = _liblib_cfg.get("starflow", {})
    if isinstance(_starflow_cfg, dict):
        MODEL_CHECKPOINT_ID = _starflow_cfg.get("checkpoint_id", MODEL_CHECKPOINT_ID)
        MODEL_TEMPLATE_UUID = _starflow_cfg.get("template_uuid", MODEL_TEMPLATE_UUID)
# --- 结束 配置覆盖 ---
...
class LiblibF1Txt2ImgAPI:
    ...
    def generate_f1_text_to_image( # 方法名保持不变，但内部逻辑针对星流模型调整
        self,
        ...
        clip_skip: int = DEFAULT_TXT2IMG_CLIP_SKIP, # 新增 clip_skip 参数
        seed: int = -1,
        ...
        save_dir: str = "starflow_txt2img_results", # 更新默认保存目录
        ...
        txt2img_params = {
            "templateUuid": MODEL_TEMPLATE_UUID, # 新增 templateUuid
            "checkPointId": MODEL_CHECKPOINT_ID, # 更新 checkpointId
            ...
            "steps": steps,
            "cfgScale": cfg_scale,
            "clipSkip": clip_skip, # 新增 clipSkip
            "seed": seed,
            "imgCount": img_count,
                    ...
                    for i, image_info in enumerate(images):
                        if image_info and isinstance(image_info, dict) and image_info.get("imageUrl"):
                            ...
                            if record_timestamp_for_filename:
                                ...
                                base_name_part = f"starflow_txt2img_{current_time_str}_seed_{image_seed}" # 更新默认文件名格式
...
if __name__ == '__main__':
    print("--- LiblibAI 星流模型 文生图 API 脚本 (已更新提示词和路径逻辑) ---") # 更新脚本标题
    ...
    # 从环境变量或配置读取凭据
    try:
        with open(os.path.join(RESOURCE_ROOT, "data", "config.json"), "r", encoding="utf-8") as _cfg_f2:
            _cfg2 = json.load(_cfg_f2)
    except Exception:
        _cfg2 = {}
    _liblib_cfg2 = (_cfg2.get("services", {}).get("liblibai", {}) if isinstance(_cfg2, dict) else {})
    YOUR_ACCESS_KEY = os.getenv("LIBLIBAI_ACCESS_KEY") or _liblib_cfg2.get("access_key", "")
    YOUR_SECRET_KEY = os.getenv("LIBLIBAI_SECRET_KEY") or _liblib_cfg2.get("secret_key", "")
    ...
    # 图片保存的基础目录（使用打包后的资源根目录，避免相对路径不一致）
    BASE_OUTPUT_DIR = os.path.join(RESOURCE_ROOT, "frontend", "static", "generated_images")
    # --- 结束配置区域 ---


    if not YOUR_ACCESS_KEY or not YOUR_SECRET_KEY: # 简单占位符/缺失检查
            ...
            if not os.path.exists(json_file_path):
                ...
                try:
                    ...
                    if isinstance(raw_list, list):
                        # 仅保留包含 '绘画内容' 的记录，防串读
                        paint_records = [r for r in raw_list if isinstance(r, dict) and str(r.get("绘画内容", "")).strip()]
                    else:
                        paint_records = []
                    print(f"成功从 {JSON_FILENAME_STEM_TO_USE}.json 读取 {len(paint_records)} 条有效绘画记录。")
            ...
            if not paint_records:
                ...
                for record_idx, record in enumerate(paint_records):
                    ...
                    if result: # 检查 result 是否为 None
                        ...
                        # 安全地获取 generateStatus
                        task_data_res = result.get("data", {})
                        generate_status = task_data_res.get("generateStatus") if isinstance(task_data_res, dict) else None
                        ...
                        elif status_code == -1 and isinstance(status_msg, str) and "超时" in status_msg:
                            print(f"  任务超时。智能体: '{agent_name}', 提示词: '{prompt_text[:50]}...'" )
                        else:
                            print(f"  文生图任务未完全成功或API返回错误。智能体: '{agent_name}'")
                            # task_data 已在上面获取为 task_data_res
                            print(f"  Task Data Status: {task_data_res.get('generateStatus') if isinstance(task_data_res, dict) else 'N/A'}, Task Message: {task_data_res.get('generateMsg')if isinstance(task_data_res, dict) else 'N/A'}")
                    else:
                        print(f"  API调用失败，没有返回结果或返回了None。智能体: '{agent_name}'")
